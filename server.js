...
async function canProceedWithRequest(limit = 100) {
  const minute = new Date();
  ...
  try {
    ...
    const current = rows[0].total_tokens || 0;
...
app.post('/chat/completions', verifyJwt, async (req, res) => {
  try {
    ...
    if (!allowed) {
      return res.status(429).json({ error: 'Token limit reached. Please wait a minute and try again.' });
    }
